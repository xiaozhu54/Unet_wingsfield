...
def write_cp_metrics_csv(path: str, rows: List[Dict[str, object]]):
    fieldnames = [
...
def analyze_samples(args):
    ...
    for sample_path in samples:
        ...
        cp_rows = summarize_cp_error(sample_name, meta, cp_true, cp_pred, cp_indices)
        cp_metric_rows.extend(cp_rows)
        for row in metrics:
            metric_rows.append({
                "sample": sample_name,
                **meta,
